...
const controlsIfMutator = {
   ...
   */
  plus: function () {
    // First click should add an explicit else. Subsequent clicks add elifs.
    if (!this.hasElse_) {
      this.addElse_();
    } else {
      this.addElseIf_();
  ...
  minus: function (index) {
    // Special case: allow removing the ELSE part explicitly.
    if (index === 'ELSE') {
    ...
    // Otherwise, remove an elif section by index or from the end.
  ...
  /**
   * Adds an explicit else input to the bottom of the block.
   * Ensures it is tracked so it persists via mutation/state APIs.
   * @this {Blockly.Block}
   * @private
   */
  addElse_: function () {
    ...
    this.hasElse_ = true;
    // Keep ELSE as the last input if additional elifs are added later.
    this.moveInputBefore('ELSE', /* put at end */ null);
  },
